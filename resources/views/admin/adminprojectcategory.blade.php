...
@section('admin_title')
Admin Pannel | Project Category | ProEducation
@endsection
...
@section('admincontent')
    ...
    <section class="content">
      <div class="box box-info">
        ...
        <form class="form-horizontal" method="POST" action="{{ url('/adminprocatregister') }}" role="form">
        {{ csrf_field() }}
          <div class="box-body">
            <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
                  <label for="name" class="col-sm-2 control-label">New Project Category : </label>
                  <div class="col-sm-5">
        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">Project Category's </h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
                	...
                	@foreach($procats as $procat)
		                <tr class="item {{ $procat->id }}">
		                  <td>{{ $procat->id }}</td>
		                  <td>{{ $procat->name }}</td>
		                  <td>{{ $procat->created_at->format('j F Y')  }}</td>
		                  <td><button type="button" class="edit-modal btn btn-block btn-info btn-sm" data-id="{{ $procat->id }}" data-name="{{ $procat->name }}" data-created_at="{{ $procat->created_at->format('j F Y') }}"><span class="glyphicon glyphicon-edit"></span></button></td>
		                  <td><button type="button" class="delete-modal btn btn-block btn-danger btn-sm" data-id="{{ $procat->id }}" data-name="{{ $procat->name }}"><span class="glyphicon glyphicon-remove"></span></button></td>
		                </tr>
	                @endforeach
                </tbody>
                <tfoot>
